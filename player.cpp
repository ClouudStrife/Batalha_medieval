...
Player::Player(int x, int y, int classe, int sentido){
	...
	if(classe == guerreiro){
        ...
        somAtaque = al_load_sample("sons/somSword.ogg");
        ...
        somAtaque = al_load_sample("sons/somFireball.ogg");
	}
        ...
        somAtaque = al_load_sample("sons/somArco.ogg");
	}
        ...
        somAtaque = al_load_sample("sons/somAxe.ogg");
	}
	...
	somRuna = al_load_sample("sons/somRuna.ogg");
	somGema = al_load_sample("sons/somGema.ogg");
	somPocao = al_load_sample("sons/somPocao.ogg");
...
void Player::atacar(Player& p2){
	...
	if(classe == guerreiro || classe == barbaro){
		if(!cooldown.estaAtivo()){
                al_play_sample(somAtaque, 1, 0, 1, ALLEGRO_PLAYMODE_ONCE, NULL);
			if(sentido == 1){
				double x1 = x;
...
void Player::usaItem(int i){
	if(i < listaItens.getTam()){
		switch(listaItens[i].getTipo()){
			case pocao:
			    al_play_sample(somPocao, 1, 0, 1, ALLEGRO_PLAYMODE_ONCE, NULL);
				vida += 20;
				if(vida > vidaMax){
					vida = vidaMax;
				}
				break;
			case runa:
			    al_play_sample(somRuna, 1, 0, 1, ALLEGRO_PLAYMODE_ONCE, NULL);
				listaProjeteis.insereNoInicio(new Projetil(x, y, 60, 30, 7, sentido, runaPoder));
				break;
			case gema:
			    al_play_sample(somGema, 1, 0, 1, ALLEGRO_PLAYMODE_ONCE, NULL);
				poder += 10;
				efeitoGema.start();
				break;
		}
