...
int main(){
	...
	ALLEGRO_AUDIO_STREAM *musicFundo = NULL, *musicBattle = NULL, *musicWin = NULL;
	...
	musicFundo = al_load_audio_stream("sons/musicMenu.ogg", 4, 1024);
	musicBattle = al_load_audio_stream("sons/musicBattle.ogg",4,1024);
	musicWin = al_load_audio_stream("sons/somWin.ogg",4,1024);
    ...
    if(estado == menu || estado == escolhaClasse || estado == escolhaCenario || estado == instrucoes){
        al_attach_audio_stream_to_mixer(musicFundo, al_get_default_mixer());
        al_set_audio_stream_playing(musicFundo, true);
        al_set_audio_stream_playing(musicBattle, false);
        al_set_audio_stream_playing(musicWin, false);
    }
    else if (estado == jogando){
        al_attach_audio_stream_to_mixer(musicBattle, al_get_default_mixer());
        al_set_audio_stream_playing(musicFundo, false);
        al_set_audio_stream_playing(musicBattle, true);
        al_set_audio_stream_playing(musicWin, false);
    }
    else if(estado == gameOver){
        al_attach_audio_stream_to_mixer(musicWin, al_get_default_mixer());
        al_set_audio_stream_playing(musicFundo, false);
        al_set_audio_stream_playing(musicBattle, false);
        al_set_audio_stream_playing(musicWin, true);
    }
